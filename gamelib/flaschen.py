...
def clampByte(i): return int(0 if i < 0 else ( i if i < 256 else 255 ))

class FlaschenDevice(object):
  '''A Framebuffer display interface that sends a frame via UDP.'''

  def __init__(self, host, port, width, height, layer=15, transparent=False):
    '''
  ...
  def write(self, buffer):
    offset = 3 + self._header_len
    for i in range(len(buffer)):
      self._data[offset + i] = buffer[i]
  ...
  def set(self, x, y, color):
    ...
    if x >= self.width or y >= self.height or x < 0 or y < 0:
      return
    if color[0] == 0 and color[1] == 0 and color[2] == 0 and not self.transparent:
      color = (1, 1, 1)

    offset = (x + y * self.width) * 3 + self._header_len
    self._data[offset] = clampByte(color[0])
    self._data[offset + 1] = clampByte(color[1])
    self._data[offset + 2] = clampByte(color[2])

    #print self._data[offset], self._data[offset + 1], self._data[offset + 2]
